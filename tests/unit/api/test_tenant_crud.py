...
def test_tenant_crud_and_idempotent_create():
    ...
    # Create tenant A
    r1 = client.post("/api/v1/tenants", json={"name": "Acme Corp"})
    ...
    r2 = client.post("/api/v1/tenants", json={"name": "Acme Corp"})
    assert r2.status_code in (200, 201)
    t2 = r2.json()
    assert t2["tenant_id"] == tid

    # List tenants
    rl = client.get("/api/v1/tenants")
    assert rl.status_code == 200
    tenants = rl.json()["tenants"]
    assert any(t["tenant_id"] == tid for t in tenants)

    # Soft delete
    rd = client.post(f"/api/v1/tenants/{tid}/delete")
    assert rd.status_code == 200
    assert rd.json()["status"] == "soft_deleted"

    # Restore
    rr = client.post(f"/api/v1/tenants/{tid}/restore")
