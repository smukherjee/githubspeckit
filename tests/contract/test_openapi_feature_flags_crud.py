...
async def test_feature_flags_crud_basic():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        tr = await client.post("/api/v1/tenants", json={"name": unique_name})
        ...
        create = await client.post("/api/v1/feature-flags", json={"flag_id": fid, "tenant_id": tenant_id, "key": unique_key, "state": "enabled"})
        ...
        # list
        listing = await client.get(f"/api/v1/feature-flags?tenant_id={tenant_id}")
